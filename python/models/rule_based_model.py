...
class RuleBasedModel:
    ...
    def fit(self, X_train, y_train=None):
        ...
        """
        try:
            self.thresholds['dos_flow_packets_per_sec_min'] = (
                X_train['Flow Packets/s'].quantile(0.95)
            )
            self.thresholds['portscan_duration_max'] = (
                X_train['Flow Duration'].quantile(0.05)
            )
    ...
    def predict(self, X_test):
        ...
        for _, row in X_test.iterrows():
            ...
            # --- PortScan ---
            if duration <= t['portscan_duration_max'] and fwd_packets <= t['portscan_fwd_packets_max']:
                label = 'PortScan'

            # --- DoS / DDoS (grouped generically here) ---
            elif flow_pps >= t['dos_flow_packets_per_sec_min'] or flow_bps >= t['dos_flow_bytes_per_sec_min']:
            ...
            # --- Heartbleed ---
            elif dst_port == t['heartbleed_port'] and flow_bps >= t['heartbleed_bytes_min']:
                label = 'Heartbleed'

            # --- Bot ---
            elif duration >= t['bot_duration_min'] and fwd_packets <= t['bot_fwd_packets_max']:
                label = 'Bot'
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_test = None, None, None  # placeholders, remove once real data is loaded
